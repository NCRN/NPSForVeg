...
#' @param output Either "dataframe" or "list". Determines the output type When \code{object} is a list. "Dataframe",the default, indicates the output from all of \code{NSPForVeg} objects should be a data.frame combining allo data as if all objects were a single park. "List" will return a \code{list} where each element of the list is a \code{data.frame} from a single \code{NSPForVeg} object, and each element is named based on that object's \code{ParkCode} slot. 
...
#' 
#' @details This function is used to calculate forestry importance values in conjcution with \code{\link{calcIV}}. One or more objects of class \code{NPSForVeg} are used. The arguments to the funciton are passed on to \code{\link{SiteXSpec}} and from there to \code{\link{getPlants}}, so any argument which is accepted by either of these fucntions (eg. \code{size.min} or \code{crown}) is valid. \code{getPlants} is used to acquire the relevent data.
