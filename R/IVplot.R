#' @title IVplot
#' 
#' @description Makes a plot of IV values
#' 
#' @param object Either a \code{data.frame} which is the output of \code{\link{IV}}, or an object of class \code{NPSForVeg}, or a list of objects of class \code{NPSForVeg}.
#' @param top  Defaults to \code{NA}. An integer. Indicates the number or species that should be included from each dataset in the graph (see \code{compare}). For a single dataset the species with a largest total IV values are selected. When multiple datasets are compared, the top species in each dataset are selected, which can result in more species than \code{top} be included in the final graph - e.g. if \code{top=3} and two datasets are used,  up to six species can be included in the graph, depeding on the amount of overlap it the top tree species. If \code{top=NA} all species are displayed.  
#' @param IVargs A required \code{list}, defaults to \code{NA}. The arguments in this list are passed to \code{\link{IV}} and are used to generate the data that are used to make the graph. Any aguments that is valid for \code{IV} can be in this list, and should be written in arguement=value format e.g. \code{IVargs=list(group="trees", years=c(2010:2013))}
#' @param parts Logical values, defaults to \code{FALSE}. Indicates if the componets of IV (Density, Size and Distribution) should be showm on the graph. 
#' @param compare A \code{list} of lists. Defaults to \code{NA}. This can be used to compare the IV valeus from seveeral different data sets, such as trees from different parks, differnt time periods or trees, sapings and seedlings from the same park. Each list will be passed on to \code{\link{IV}}, and can contain any arguments that are valid for that funciton. Entries in the list should be written in argument=value format  e.g. \code{compare=list( list(object=CATO, group="saplings", years=c(2010:2013)), list(object=CATO, group="seedlings", years=c(2010:2013)) )} Any number of datasets can be added using \code{compare}.
...
#' 
#' @details This fucntion uses the \code{barchart} function in the \pkg{lattice} package to create a horizontal barchart that displays IV values. The foramt of the display varies depending on the type of data to be displayed. If only a single dataset is considered (\code{compare=NA}), and \code{parts=FALSE}, then a horizontal bar chart is produced. If \code{parts=TRUE} a stacked bar chart is produced. Each component of IV: density, size and distribuiton, are shown in a different color. 
#' 
