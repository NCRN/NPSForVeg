...
#' @import reshape2
#' 
#' @description Produces a Site X Species matrix. Each cell can contain a count, a measure of size or 1/0 indciating presence/abscence
#' 
...
#' @param valeus Determines the data contained in the Site X Species matrix. Possible values are:
#' \describe{
...
#' \item{"size"}{For trees and saplings this is the total basal area. For tree seedlings and shrub seedlings it is the total height, and for herbs it si the total percent cover across all quadrats. For shrubs and vines there is no defined size and the function will terminate with an error.}
#' \item{"presab"}{Produces a presrnce/absence matrix. When a plant species is preseent in a given plot the corresponding cell value will be one, otherwise it is zero.}
#' }
#' @param output Either "dataframe" or "list". Determines the output type When \code{object} is a list. "Dataframe",the default, indicates the ouput from all of \code{NSPForVeg} objects should be a single lareg matrix, containing all sites and species from all \code{NPSForVeg} objects. "List" will return a \code{list} where each element of the list is a \code{data.frame} from a  single NSPForVeg obeject, and each element is named based on that objects \code{ParkCode} slot. 
...
#' 
#' @details This function will first call \code{\link{getPlants}} to retrieve the plant data. Then a SiteXSpecies matrix will be creaated. Values in the cells are determine by the option selected with \code{values}. Each row corresponds to a different plot and each column to a different column. 
#' 
              ...
              size={
                switch(group,
                  ...
                  trees=,saplings=OutData<-dcast.data.table( setkey(XPlants,fPlot,Latin_Name)[CJ (unique(levels(fPlot)), unique(Latin_Name)),
                                      sum(SumBasalArea_cm2)],formula=fPlot~Latin_Name,value.var="V1", drop=FALSE),
