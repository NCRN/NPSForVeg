...
#' }
#' @param density Deprecated. No longer does anyting, use \code{area} instead. 
#' @param species Character vector, defaults to \code{NA}. Passed on to \code{\link{SiteXSpec}} and from there to \code{\link{getPlants}} to limit the output to one or more designated species.
#' @param plots Character vector, defaults to \code{NA}. When not \code{NA} indicates which plots should be used in the analysis. Otherwise all plots are used.
...
#' }
#' @param plotarea Single number. Defaults to \code{NA}. Used when the input object is a \code{list} and the output is a \code{data.frame} and is on a per plot basis.
...
#' If \code{values="presab"} data is treat as being binomial and confidence intervals are determined using \code{glm}.  
#' 
...
#' subplot is not sampled during an event, perhaps due to a safety concern, then the analysis needs to take into account that some of the
#' data is missing. Similarly, if the design of the plot changes, adding or removing subplots or changing the area sampled similar problems occur.
#' Finally, if you are combining data from areas with different plot designs, this also must be taken into account.
#' 
...
#' if left as \code{NA}
#'
#' When the input is a list, \code{output="list"} data from multiple parks are combined. Differences in sampled area are accounted for
...
setGeneric(name="dens",function(object,group, years, values="count", density=TRUE,species=NA, plots=NA, area="plot", plotarea=NA, subplots=NA,
