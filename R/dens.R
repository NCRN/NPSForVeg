...
setMethod(f="dens", signature=c(object="NPSForVeg"), 
         function(object,...){
          TempData1<-SiteXSpec(object=object,group=group,years=years,values=values, species=species,...)
          XSubplots<-getSubplotCount(object=object, group=group,years=years, subtype='all')
          TempData<-merge(XSubplots[,c('Plot_Name','numSubPlots')],TempData1, by=('Plot_Name')) %>% filter(numSubPlots>0)
          OutData<-data.frame(Latin_Name=names(TempData[-c(1:2)]),Mean=NA,Lower.95=NA, Upper.95=NA)
          
          switch(values,
                 count={
                    NBS<-lapply(X=names(TempData[-c(1:2)]), FUN=function(Y,Z){glm.nb(Z[[Y]]~1+offset(log(TempData$numSubPlots)))}, Z=TempData[-c(1:2)])
                    OutData$Mean<-sapply(X=NBS,FUN=function(x){max(fitted.values(x))})
                    NBS<-lapply(X=names(TempData[-c(1:2)]), FUN=function(Y,Z){glm.nb(Z[[Y]]~1)}, Z=TempData[-c(1:2)])
                    OutData[3:4]<-matrix(sapply(X=NBS, FUN=function(x){suppressMessages(confint(x))}),ncol=2,byrow=TRUE)
                    OutData[3:4]<-exp(OutData[3:4])
                  },
                  size={
                    OutData$Mean<-sapply(X=names(TempData[-c(1:2)]), FUN=function(X,Z){mean(Z[[X]])}, Z=TempData[-c(1:2)])
                    SzBoot<-list()
                    SzBoot<-lapply(
                      X=names(TempData[-c(1:2)]), Z=TempData[-c(1:2)], FUN=function(X,Z){
                 ...
                 presab={
                   ...
                   BIN<-lapply(X=names(TempData[-c(1:2)]), FUN=function(Y,Z) {glm(Z[[Y]]~1, family=binomial)}, Z=TempData[-c(1:2)])
          ...
          if(density & group!="herbs" & values=="count") {
