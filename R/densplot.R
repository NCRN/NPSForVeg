...
#' @param Total Logical, defaults to \code{TRUE}. Indicates if the the total values for all species, such as total numbers of seedligns, should be plotted. 
#' @param top Numeric, defaults to \code{NA}. When not \code{NA} only the number of species indicated in \code{top} will be used for each dataset (see \code{comnpare}). If several datasets are used, then more than \code{top} species can be plotted, if the most common species are not the same in all datasets.
...
#' 
#' 
#' @details This fucntion uses the \code{xyplot} function in the \pkg{lattice} package to create plots that display means and 95\% confidence inervals for a varielty of measures of plants: - abundance, basal area, cover, occupancy. The format of the display varies depending on the type of data to be displayed. If only a single dataset is considered (\code{compare=NA}), then each species will be displayed. 
#' 
...
setMethod(f="densplot",signature=c("NPSForVeg"),
          function(object, Total, top, densargs,...){
            ...
            if(all(!is.na(compare))){
              ...
              Tempdens<-ldply(.data=dens2, .fun=make.groups)
