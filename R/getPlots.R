...
setGeneric(name = "getPlots", function(object, type = "active", visits = NA, years = NA, plots = NA, subparks = NA, events = NA, output = "dataframe") {
...
setMethod(
  ...
  function(object, type, visits, years, plots, subparks, output) {
    ...
    OutPlots <- switch(output,
      list = map2(.x=XPlots, .y=events, .f= ~ getPlots(.x, type, visits, years, plots, subparks, events=.y)),
      dataframe = getPlots(object = bind_rows(XPlots), type, visits, years, plots, subparks, events)
    )
    return(OutPlots)
  }
)
...
setMethod(
  ...
  function(object, type, visits, years, plots, subparks) {
    XPlots <- object@Plots
    ...
    OutPlots <- getPlots(XPlots,
      type = type, visits = visits, years = years, plots = plots, events = events,
      subparks = subparks
    )
...
setMethod(
  ...
  function(object, type, visits, years, plots, subparks, events) {
    object <- switch(type,
      all = object,
